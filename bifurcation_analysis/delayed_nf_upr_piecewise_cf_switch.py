...
running the delay differential equation model for the Cf-switch activation mechanism. The
...
def steady_state(params,active=0):
    ...
    if active==1:
        ...
        Y = (G*cmax*m+1.)*alpha
        Z = G*m*beta
        A = np.sqrt(X**2.*beta**2 + 2.*X*(Y+nu)*beta + (Y-nu)**2.)
        uss = 0.5*(-G*m*(alpha*cmax+beta) + A + (nu-alpha-beta))
        c_numerator = -(Y+Z)*A + Y**2. + (2.*Z + beta - nu)*Y + Z*(Z + beta + nu)
        c_denominator = alpha*(-A + Y + Z + beta - nu)
        css = c_numerator/c_denominator
        #print css
    if active==2:
        css = 1.+G
